...
#ifndef CAMERAMODELS_METASHAPE_H
#define CAMERAMODELS_METASHAPE_H
...
public:
    Metashape()
    {
        mvParameters.resize(11);
        mnId = nNextId++;
        mnType = GeometricCamera::CAM_METASHAPE;
    }
    Metashape(const std::vector<float> _vParameters) : GeometricCamera(_vParameters), tvr(nullptr)
    {
        assert(mvParameters.size() == 11);
        mnId = nNextId++;
        mnType = GeometricCamera::CAM_METASHAPE;
    }
    Metashape(Metashape* pMetashape) : GeometricCamera(pMetashape->mvParameters), tvr(nullptr)
    {
        assert(mvParameters.size() == 11);
        mnId = nNextId++;
        mnType = GeometricCamera::CAM_METASHAPE;
    ...
    friend std::ostream& operator<<(std::ostream& os, const Metashape& ms);
    friend std::istream& operator>>(std::istream& is, Metashape& ms);

    bool IsEqual(GeometricCamera* pCam);

private:
    //Parameters vector corresponds to
    // [fx, fy, cx, cy, k1, k2, k3, k4, p1, p2, skew]
    TwoViewReconstruction* tvr;
};
}  // namespace ORB_SLAM3

#endif  //CAMERAMODELS_METASHAPE_H
